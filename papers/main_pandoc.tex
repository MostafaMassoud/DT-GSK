...
\title{DT-GSK: Dimension-Tiered Adaptive Configuration Selection and Deterministic Refinement for Gaining-Sharing Knowledge-Based Optimization Algorithm}
...
@@SUP!1@@ \quad Department of Operations Research, Faculty of Graduate Studies for
...
moustafa.masoud@gmail.com (M.E.A.M.\ ORCID 0009-0003-8415-2158); hebasayed@cu.edu.eg (H.S.M.R.\ ORCID 0000-0003-0387-5876); aliwagdy@gmail.com (A.W.M.\ ORCID 0000-0002-5895-2632)\\
@@SUP!2@@ \quad Mathematics Program, University of Science and Technology,
Zewail City of Science and Technology, October Gardens, 6th of October,
Giza 12578, Egypt\\
...
\item \textbf{C2: a dimension-tiered adaptive scaffold.} The
  ...
  disclosed as mis-specified (Section~@@REF!sec:alg:scaffold@@). 
...
\subsection{@@SECNUM!sec:rel:family!2.1.@@ The GSK Family: From Fixed Parameters to Adaptive Configuration}\label{sec:rel:family}
...
The tie-accepting form above is the base rule and is what runs at $D<100$. At $D\ge100$ the late-acceptance clip can replace it, late in the budget and after a stall streak, with the strict test $f(v_i) < f(x_i) - \epsilon_{\mathrm{clip}}\lvert f(x_i)\rvert$ at $\epsilon_{\mathrm{clip}}=10^{-3}$ (a distinct symbol from the stagnation tolerance $\epsilon$), which rejects ties; the clip's own gates are given in Supplementary Section~S5.
...
8. Deterministic final polish (Eq.~(@@REF!eq:eigen-polish@@)) & gated & $D{\ge}50$ & one-shot, RNG-free compass search on the ISM eigenbasis, in the final budget slice \\
\bottomrule
\end{tabular}
\end{table}
...
algorithm spends that budget rather than in how much it is given. Both
settings are protocol-conformant: each suite's protocol fixes its evaluation
budget and run count (Table~@@REF!tab:protocol@@) and prescribes no population
size. We state it
...
know the initial populations differ by a factor of five. A common-initial-$NP$
replication was carried out on CEC2017, with \dtgsk{}'s own reduction law and
tier floors retained: at the comparators' $NP = 100$, \dtgsk{} remains first
...
Section~S9.2). On CEC2013LSGO the corresponding ratio at initialization is
fifty-fold; it is disclosed but not controlled (Supplementary Materials,
Section~S7).
...
@@ALGLINE!-!0@@ \textbf{Require:}  objective $f$, dimension $D$, bounds $[\ell,u]$, budget $\mathrm{MaxFES}$, seed \ \ $\triangleright$~budgets: Section~@@REF!sec:alg:complexity@@

@@ALGLINE!-!0@@ \textbf{Ensure:}  global best solution $x^{gb}$

@@ALGLINE!-!1@@ \emph{Initialization}

@@ALGLINE!1!0@@ Initialize the interaction graphs $G_{\mathrm{abs}}, G_{\mathrm{signed}} \gets 0$ \ \ $\triangleright$~allocated only at $D\ge50$
...
@@ALGLINE!8!1@@ Assign each individual an ACE arm \ \ $\triangleright$~$(KF, KR, K_{\exp})$; a sixth DE arm at $20{\le}D{<}100$
...
@@ALGLINE!12!1@@ Form the crossover mask: linkage block on some rows, $KR$ mask on the rest \ \ $\triangleright$~Eq.~(@@REF!eq:kr-mask@@)

@@ALGLINE!13!1@@ \textbf{if} DE arm: overwrite the masked coordinates with a DE/rand/1 mutant \ \ $\triangleright$~$F{=}0.5$, $CR{=}0.9$

@@ALGLINE!14!1@@ Repair the out-of-bound coordinates

@@ALGLINE!15!1@@ Evaluate the maximal prefix of $\{v_i\}$ that fits the budget \ \ $\triangleright$~the rest are non-selectable

@@ALGLINE!16!1@@ Greedily keep each no-worse trial and update the diversity archive \ \ $\triangleright$~Eq.~(@@REF!eq:greedy-accept@@)

@@ALGLINE!-!2@@ \quad\emph{Learn, escape, and refine \textnormal{(dimension-gated)}}

@@ALGLINE!17!1@@ \textbf{if} $D{\ge}50$: update the interaction-structure memory \ \ $\triangleright$~Eq.~(@@REF!eq:sgsm-graph@@)

@@ALGLINE!18!1@@ Update the ACE credit and prune stalled arms (ARGP)

@@ALGLINE!19!1@@ \textbf{if} stagnating: apply the budget-safe escape \ \ $\triangleright$~Eq.~(@@REF!eq:bse-cauchy@@)

@@ALGLINE!20!1@@ \textbf{if} in the endgame slice: charged coordinate local search on the elites

@@ALGLINE!21!1@@ \textbf{if} $D{\ge}50$: polish the working incumbent $x^{*}$ once, in the final slice \ \ $\triangleright$~Eq.~(@@REF!eq:eigen-polish@@)

@@ALGLINE!-!2@@ \quad\emph{Update and restart}

@@ALGLINE!22!1@@ Update the global best $x^{gb}$

@@ALGLINE!23!1@@ \textbf{if} deep-stalled: restart the population, keeping $x^{gb}$

@@ALGLINE!24!0@@ \textbf{end while}

@@ALGLINE!25!0@@ \textbf{return} $x^{gb}$

@@ALGLINE!-!1@@ \emph{Note.}~\textnormal{The linkage block is learned at $D{\ge}50$ (random otherwise) and confidence-gated; the local search enters at $0.80$ of the budget below $D{=}50$ and at $0.70$ above, under its evaluation cap and cooldown; and the $D{\ge}100$ upper-tier controllers are cross-cutting (they act inside the reduction, trial-construction, acceptance and escape steps above and gate the local search) as detailed in Section~@@REF!sec:algorithm@@.}
...
and the one-shot deterministic final polish. At $D\ge100$ the upper-tier controllers are cross-cutting rather than a single stage: they act within the population-reduction, trial-construction, acceptance, and escape steps above (and gate the local search), so they are not drawn as a separate node. The global best is preserved
throughout.}
...
\begin{enumerate}[itemsep=2pt,topsep=4pt,leftmargin=*,label=(\arabic*)]
...
\item At $D\ge50$, the one-shot deterministic final polish in the final
budget slice.
...
deterministic final polish activate at $D\ge50$, while the coordinate local search
...
Deterministic final polish (Eq.~(@@REF!eq:eigen-polish@@)) & off & off & on & on & one-shot, start $0.96$ \\
...
not isolated by the component studies above. A controlled
tiered-versus-tier-constant contrast (Supplementary Materials,
Section~S9.3) separates the
...
$g_{\mathrm{act}}\in\mathbb{R}^{D}$ & per-dimension activity: improvement-weighted, $\ell_1$-normalized $\lvert\Delta x\rvert$ accumulated linearly in a separate vector (the graph diagonals themselves are zeroed); decaying; LS dim-selection, inactive in the frozen config\\
...
$\operatorname{conf}(G_{\mathrm{abs}})$ & graph confidence $\kappa(\cdot)$: a block-size-weighted mean of per-block scores in edge density, magnitude, support and stability, scaled by the square root of the non-trivial-dimension coverage (Supplementary Materials, Section~S5)\\
...
blocks every 5 generations at $D=50$ and every 20 at $D=100$. 
...
that basis is an implementation choice the direct isolation does not
...
directions. The three-arm contrast (Supplementary
...
No per-suite or per-run tuning is performed: every value here is resolved from $D$ alone and is identical across the four bound-constrained suites. The adapter does expose an advanced per-run override path, which is not used for any result in this paper.}
...
$KR$ & 0.9 (ACE arm-controlled) & block mask when linkage on; floor $\kappa_{\dim}$ (S5)\\
$K_{\exp}$ & 10 (ACE arm-controlled) & junior-dimension schedule\\
$p$ (senior) & 0.05 ($0.15$ at $20$--$49$) & at $D\ge50$ worse half $\to0.10$, only while acc.\ ${\le}0.10$\\
\midrule
...
a Holm-significant loss to \egsk{}. That aggregate rank is a descriptive
summary, not evidence of overall superiority: on CEC2017 the Holm-corrected
pairwise tests separate \dtgsk{} from \egsk{} only at $D = 10$, and the two
are never Nemenyi-separable at any dimension of that suite
(Section~@@REF!sec:exp:cec2017@@). The CEC2020 and CEC2013LSGO legs,
...
(SciPy~1.15.3) runs
restricted so that it can read only the immutable evidence releases
identified in the Data Availability Statement and the Supplementary
Materials, never staging data and never
...
ranges carry more weight in it. This cross-function scale heterogeneity is a
genuine limitation
...
$=$ no significant difference, $-$ worse).
...
Figure~@@REF!fig:cd-cec2017@@ shows the Nemenyi critical-difference view
(one panel per dimension, the within-one-CD-of-best cohort shaded), emitted only because the omnibus is significant at every
...
\caption{@@FIGCAP!fig:cd-cec2017@@ Critical-difference view of the CEC2017 GSK-family Friedman mean
ranks (lower is better), one panel per dimension.  Bars are mean ranks; the
shaded rows are the cohort within one critical difference of the best rank,
and the dashed line marks best rank $+$ CD ($\mathrm{CD} = 1.67$ for
$k = 7$, $N = 29$, $\alpha = 0.05$).  A bar crossing the line is
Nemenyi-separated from the best; \dtgsk{} and \egsk{} fall inside one
cohort at every dimension.}\label{fig:cd-cec2017}
...
ordinal positions (1/2/1/1 at $D = 10/30/50/100$) are identical under
both of these robustness variants, but panel orderings between comparators are not fully
...
$p_{\mathrm{Holm}} = 4.2 \times 10^{-2}$). It is the study's only
Holm-significant unfavorable cell on the three bound-constrained comparison
suites; the pre-registered CEC2020 leg adds two at $D = 20$
(Section~@@REF!sec:exp:cec2020@@). It
...
result existed, at the signing of the registered
...
every dimension-gated \dtgsk{} subsystem is inactive at $D \leq 20$)
and is not affirmative evidence for the tiering choice itself.

``Inactive'' here follows the gating taxonomy of
Table~@@REF!tab:dim-gating@@: at the $D = 20$ cells the always-on scaffold
does cross into its $20$--$49$ tier (the optional
differential-evolution arm joins the ACE pool, the credit assignment
splits into a top/bottom improvement memory, and several tier constants
change) while the gated subsystems themselves (the
interaction-structure memory, the deterministic final polish, and the
upper-tier controllers) remain off.
...
floor; best mean rank per dimension column in bold) and the descriptive
across-dimension aggregate. No test is attached to
...
competition~@@CITE!mohamed2020agsk@@, and its paper is a co-author's.
...
Both convergence figures share one reading basis, stated once here rather
than repeated in each caption: panels follow the fixed, prespecified
function-selection rule frozen before rendering (each set includes a mandated
unfavorable case); every curve is the per-checkpoint mean error over the 51
runs on an identical checkpoint grid with no smoothing; the $y$-axis is
log-error with a display-only floor of $10^{-14}$ for exact zeros, which
never enters any statistic; and one legend serves all panels.

\begin{figure}[H]
\includegraphics[width=0.92\textwidth]{figures/convergence/main_cec2017_D30.png}
\caption{@@FIGCAP!fig:conv-cec2017-d30@@ Family-overlay convergence on CEC2017 at $D = 30$ (
\dtgsk{}'s known-weak tier, ranked second behind \egsk{}) for the four
prespecified functions: F3 (unimodal; easy/strong), F10 (simple multimodal;
hard/comparable), F12 (hybrid; hard/comparable), F26 (composition;
hard/weak, the mandated unfavorable case). Shared basis as stated above;
APGSK checkpoint evidence at $D = 30$ carries the per-run availability
qualification of Section~@@REF!sec:exp:settings@@.}\label{fig:conv-cec2017-d30}

\end{figure}

\begin{figure}[H]
\includegraphics[width=0.92\textwidth]{figures/convergence/main_cec2017_D100.png}
\caption{@@FIGCAP!fig:conv-cec2017-d100@@ Family-overlay convergence on CEC2017 at $D = 100$, the tier where
the structure-memory and polish subsystems are active, for the four
prespecified functions: F1 (unimodal; easy/strong), F5 (simple multimodal;
easy/strong), F12 (hybrid; hard/comparable), F26 (composition;
hard/comparable). Shared basis as stated above.}\label{fig:conv-cec2017-d100}
...
Section~S6.7), and it is not small: enabling the memory costs $+57.3\%$
wall-clock on CEC2017 at $D = 50$, $+36.3\%$ at $D = 100$, and $+30.3\%$ on
CEC2013 at $D = 50$, against no detectable standalone accuracy return at the
tiers where it is active. On the other suites \dtgsk{}'s mean per-run time is
...
anything learned during the run. The three-arm isolation of Supplementary
Materials Section~S9.1 bears on this explanation and supports it: the coordinate
arm alone beats no refinement at both active dimensions. Second, \dtgsk{} begins from
$NP = 5D$ against the comparators' $NP = 100$, a five-fold difference
at $D = 100$. The comparators' value is the family's
reference-implementation setting. A matched-initial-population control
isolates the asymmetry (Supplementary Materials,
Section~S9.2): \dtgsk{} stays within the top two of the panel at every
dimension at the common value, though its $D = 50$ and $D = 100$ first places
...
before any of its data existed (the registered addendum was signed, and its
checksum bound into the evidence release, before the first CEC2020 result;
Section~@@REF!sec:exp:cec2020@@ and
...
The upper-tier ($D \geq 50$) behavior (Figures~@@REF!fig:cd-cec2017@@
and~@@REF!fig:conv-cec2017-d100@@; Table~@@REF!tab:friedman-cec2017@@) arises under
...
tiers, at a tier-dependent compute overhead; the basis isolation
(Supplementary Materials Section~S9.1) finds the
memory's learned eigenframe outperformed by the coordinate axes in the polish
at $D = 50$. Per-component attribution to
...
evaluates the low-dimensional boundary condition of the tiered design (
every dimension-gated \dtgsk{} subsystem is inactive at $D \leq 20$)
and is not affirmative evidence for the tiering choice itself. That suite is the pre-registered confirmatory
...
Section~S9.4). It is descriptive and carries no hypothesis test. A
pre-registered boundary study (Supplementary Materials,
Section~S9.5) varies the tier boundaries themselves: the standing is
insensitive to the tested shifts at $D = 10$ and $D = 50$, while at $D = 30$
both neighbouring profiles beat the shipped middle set and at $D = 100$ the
$50 \leq D < 100$ set beats the shipped upper profile on paired means with
the family ordinal unchanged (boundary-level findings, attributed to no
individual mechanism, that extend the disclosed mis-specification).
...
A configuration-transplant experiment locates part of that
...
Section~S9.3), so the shipped $20 \leq D < 50$ configuration is inferior to
the tested low-tier transplant at that dimension. This localizes a
configuration-level weakness; it does not identify the responsible key, and
the same experiment demonstrates tiering against a high-dimension transplant
at $D = 10$ and $D = 50$, which is why contribution~C2 is claimed for those
two dimensions only. Which of the differing configuration keys carries
the effect is not resolved here and is left open.
...
$X_{0}$ used by the six comparators. That departure is bounded rather than
merely disclosed: held to the comparators' initial $NP = 100$, \dtgsk{}
stays within the panel's top two at every CEC2017 dimension, but its first
places at $D = 50$ and $D = 100$ do not survive, so those two rank claims
rest in part on the population rule (Supplementary Materials,
Section~S9.2).
...
uncontrolled implementation and environment differences into the comparison. Attribution gaps bound what the
component evidence can support; the basis gap among them is closed by a
direct three-arm isolation (Supplementary Materials, Section~S9.1), which
separates the deterministic endgame
(supported) from the learned basis it searches (not supported), leaving open
alternative bases, other suites, and the conditions under which accepted-move
geometry becomes informative. Finally, the
...
A direct basis isolation sharpens this. In the
...
$D = 50$ ($p_{\mathrm{Holm}} = 1.4\times10^{-4}$, 25 of 29 functions) and, under
the manuscript's stated tie rule, at $D = 100$ as well
($p_{\mathrm{Holm}} = 0.0489$), while the
polish itself beats no refinement at both active dimensions (Supplementary
Materials, Section~S9.1). At both active dimensions, therefore,
...
for this reason.
Both mechanisms are retained unchanged. The configuration evaluated here was
fixed and checksum-locked before these isolations were run, so replacing a
component in response to the experiment that measured it would define a
different algorithm, and every number reported here is bound to the one that
was evaluated. The isolations therefore correct the claims rather than the
method, and the mechanisms remain in the shipped configuration so that both
negative results are reproducible from it.
...
Conceptualization, M.E.A.M.;
methodology, M.E.A.M. and A.W.M.; software, M.E.A.M.;
formal analysis, M.E.A.M. and A.W.M.; investigation, M.E.A.M. and A.W.M.;
data curation, M.E.A.M. and A.W.M.;
writing---original draft preparation, M.E.A.M. and A.W.M.;
...
\noindent\textbf{Data Availability Statement:} The source code,
...
tag v2.37, and any further materials are
...
SHA-256 checksum for every released file. The CEC2013LSGO and CEC2020
suites, the five revision experiments (E1--E5), and the identity
re-execution behind the Supplementary Materials Table~A45 residual are
carried by five further additive, non-superseding releases on the same
terms, each with its own checksummed manifest and none changing any
primary value; their identifiers are recorded in the Supplementary
Materials and in the repository's release manifests. The derived
statistical artifacts behind every table and figure are versioned as
checksummed analysis bundles alongside them. The supplementary component-isolation study
...
During the preparation and revision of this manuscript, the
...
supervisor of author M.E.A.M.  One adjacency is disclosed specifically:
...
@@BIBENT!kolda2003directsearch!27@@ Kolda, T.G.; Lewis, R.M.; Torczon, V. Optimization by Direct Search: New Perspectives on Some Classical and Modern Methods. \emph{SIAM Review} \textbf{2003}, \emph{45},~385--482. doi:{\href{https://doi.org/10.1137/S0036144502428893}{10.1137/S0036144502428893}}.
