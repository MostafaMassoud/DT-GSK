...
\midrule
Inherited GSK core (Eqs.~(\ref{eq:junior-idx})--(\ref{eq:gsk-update}), (\ref{eq:midpoint-repair})) & inherited & all & junior/senior gaining-sharing vector update and midpoint repair (numerics unchanged); the crossover mask and greedy-selection ties are modified (rows~3,~5; Eq.~(\ref{eq:greedy-accept})) \\
...
4. ISM interaction-structure memory (Eq.~(\ref{eq:sgsm-graph})) & gated & $D{\ge}50$ & strictly-improving-accepted-move EMA graph; acts when $\operatorname{conf}(G_{\mathrm{abs}})\ge\kappa_{\min}$ \\
...
published defaults rather than at a common population size. The six
comparators use $NP=100$ at every dimension, as specified in their source
...
\midrule
Inherited GSK core (Eqs.~(\ref{eq:junior-idx})--(\ref{eq:kr-mask}), (\ref{eq:midpoint-repair}), (\ref{eq:greedy-accept})) & on & on & on & on & senior $p$ $0.05$ ($0.15$ at $20$--$49$), split-bottom $0.10$ at $D{\ge}50$ \\
...
\emph{soft-frozen} to the probability floor $p_{\min}=0.05$ rather than removed,
...
generations at $D=50$ and every 20 at $D=100$. % BIND: phase_03/parameter_table.md (block size by tier) + phase_03/contribution_matrix.md row 8
...
\midrule
GSK & \cite{mohamed2020gaining} & published constants, $NP=100$ at every $D$; in-repository re-run\\
AGSK & \cite{mohamed2020agsk} & published constants, $NP=100$ at every $D$; in-repository re-run\\
APGSK & \cite{apgsk2021} & published constants, $NP=100$ at every $D$; in-repository re-run\\
...
FDB-AGSK & \cite{fdbagsk2023} & published constants, $NP=100$ at every $D$; in-repository re-run\\
ATMALS-GSK & \cite{alfadli2025atmals} & published constants, $NP=100$ at every $D$; in-repository re-run\\
...
\dtgsk{} is omitted from this table because its complete frozen
configuration is already given in Table~\ref{tab:parameters}; its single
