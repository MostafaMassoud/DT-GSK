...
% Row striping removed at the authors' request, for consistency with the
% MDPI table style. \zebra is kept as a no-op so every table that calls it
% is unaffected structurally; restoring the stripe is a one-line edit.
\newcommand{\zebra}{}
...
\Title{Supplementary Material for: DT-GSK: Dimension-Tiered Adaptive Configuration Selection and Deterministic Refinement for Gaining-Sharing Knowledge-Based Optimization Algorithm}
...
% matching main.tex; a new affiliation 2 (Zewail City) for A.W.M. added at the
% author's direction on 2026-08-29 (CR-0038), matching main.tex.
...
\centering
\tablesize{\footnotesize}
\resizebox{\textwidth}{!}{\input{tables/T07}}
\end{table}
...
\centering
\tablesize{\footnotesize}
\resizebox{\textwidth}{!}{\input{tables/T08}}
\end{table}
...
\centering
\tablesize{\footnotesize}
\resizebox{\textwidth}{!}{\input{tables/T10}}
\end{table}
...
\centering
\tablesize{\footnotesize}
\resizebox{\textwidth}{!}{\input{tables/T02}}
\end{table}
...
\centering
\tablesize{\footnotesize}
\resizebox{\textwidth}{!}{\input{tables/T03}}
\end{table}
...
\centering
\tablesize{\footnotesize}
\resizebox{\textwidth}{!}{\input{tables/T05}}
\end{table}

% Flush the deferred CEC2017 per-function and head-to-head [p] floats here,
% so this subsection's inline [H] summary table cannot render on an earlier
% page than the floats that precede it in numbering.
\clearpage
...
\centering
\tablesize{\footnotesize}
\resizebox{\textwidth}{!}{\input{tables/T01}}
...
\centering
\tablesize{\footnotesize}
\resizebox{\textwidth}{!}{\input{tables/T11}}
\end{table}
...
\centering
\tablesize{\footnotesize}
\resizebox{\textwidth}{!}{\input{tables/T12}}
\end{table}
...
\centering
\tablesize{\footnotesize}
\resizebox{\textwidth}{!}{\input{tables/T13}}
\end{table}

% Flush the deferred CEC2011/CEC2013 per-problem and per-function [p] floats
% before the two inline [H] summary tables below, so page order matches
% table numbering.
\clearpage
\begin{table}[H]
...
\resizebox{\textwidth}{!}{\input{tables/SA03}}
\end{table}
...
\input{build_prompt_phases/phase_03/parameter_table_detail.tex}
...
% Flush the deferred CEC2020 Wilcoxon and effect-size [p] floats here, so the
% inline [H] robustness table below cannot render on an earlier page than the
% floats that precede it in numbering.
\clearpage
...
\resizebox{\textwidth}{!}{\input{tables/SA05}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/SA06}}
\end{table}
...
with zero seed mismatches).}\label{tab:rev-e3}
\resizebox{\textwidth}{!}{\input{tables/SA07}}
\end{table}
...
A1).}\label{tab:rev-e4}
\resizebox{\textwidth}{!}{\input{tables/SA08}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/SA09}}
\end{table}
...
% BEFORE the end of the document, so they emit as one contiguous block
% rather than as a four-page float backlog against the back matter
% (panel round-2 finding S6-PDF-001). The supplement carries no reference
% list: all citation apparatus was removed at pass-64 per the editorial
% office request of 2026-09-01 (MDPI: supplements are standalone).
\clearpage
