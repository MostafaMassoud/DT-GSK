...
\title{Supplementary Material for: DT-GSK: Dimension-Tiered Adaptive Control and Deterministic Refinement for Gaining-Sharing Knowledge Optimization}
...
confirmatory suite record (Section~S8).  Every empirical value in Sections~S1--S5 derives from the
...
ablation release; the Section~S7 and Section~S8 values derive from two
...
$M$ & maximum safe seed (child-seed modulus)\\
\bottomrule
\end{tabular}
\end{table}
...
substitutes the published fmincon-based solver. Cross-implementation
...
$D \le 100$ on CEC2017 and CEC2013, native dimensions on CEC2011,
...
\emph{learned} eigenbasis it consumes: whether that basis beats coordinate
axes or a matched random orthonormal basis is not isolated by any contrast
reported here and remains open. And the evidence-bearing runs carry no
...
\midrule
ACE & Adaptive Control Engine (operator-configuration selector)\\
...
init probs & $(0.05,0.05,0.45,0.05,0.40)$ & \\
...
restart response & matrix decay 0.50 (interaction\_restart\_decay); post-restart cooldown 5 gen.\ & applied on each BSE/deep-stall restart\\
\midrule
...
trust-region budget policy & present, runtime-inert & redundant guard; not an active mechanism\\
\bottomrule
\end{tabular}
\end{table}
...
enters with initial probability $0.10$--$0.20$ by tier.  When a generation yields no net
...
is tracked separately).  The per-source weight $s_i$ is $0$ for
...
$s\cdot D^{-1/2}\cdot\mathrm{span}\cdot\mathcal{C}(0,1)$, with scale
$s=0.04$ at $D\ge 20$ and $s=0.05$ at $D<20$; the escape is active at all
tiers (the $D<20$ tier enables it through the low-dimension escape
overrides).  Finally, the RNG substream
modulus of main-text Equation~(13) is $M = 2\,147\,483\,646$: stream~$0$ is the run seed
...
is the \emph{summed} (not averaged) accepted improvement of the individuals that drew
...
it merely slows the L-SHADE-style population reduction.  One reproducibility coupling is
worth stating explicitly: the fitness-affecting link-lift EMA read-back that feeds the
senior-widening and broaden gates is updated inside the per-generation callback path, and
the released driver's adapter always supplies that callback, so the read-back is active in
every reported run; a callback-less direct invocation of the core would omit it and
diverge at $D\ge 100$, but no released configuration does so.
...
published MATLAB fmincon polish, with no numerical-equivalence
...
per-function mean final error (lower rank is better);
...
At $D=100$ the Friedman omnibus signal is weakest ($p = 3.5\times10^{-2}$) and
...
runs; the four cells differ overall (Friedman $p = 2.4\times10^{-3}$,
$5.2\times10^{-3}$, and $3.8\times10^{-3}$ for CEC2017 $D50$, CEC2017 $D100$,
and CEC2013 $D50$).  These component-study omnibus $p$-values are the
classical Friedman chi-square approximation (no tie-variance correction is
applied in these component studies), not the Iman--Davenport $F$
that serves as the omnibus decision criterion for the primary suites in the
main text.  The two statistics are not interchangeable, so these figures
should not be read against main-text omnibus values.
