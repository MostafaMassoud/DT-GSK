...
confirmatory suite record (Section~S8), and the four round-one revision
experiments (Section~S9).  Every empirical value in Sections~S1--S5 derives from the
...
ablation release; the Section~S7, Section~S8 and Section~S9 values derive from three
further separate, non-superseding evidence releases identified in those
sections; no primary result is
...
primary, \dtgsk{}'s own ordinal position is identical in every
variant reported here, but panel orderings between comparators are not fully robust.
...
pins. No reported number, rank, $p$-value or decision in this paper or this
Supplementary Material depends on which of the two revisions is used; the
release remains reproducible from either.
...
frozen panel's 51, and it perturbs no tier threshold; the tier constants
themselves remain frozen and hash-locked.  Neither ARGP nor the deep-stall
...
improvement (its summed $f_{\mathrm{old}}-f_{\mathrm{new}}$ is non-positive) the
...
\item \textbf{Eligibility to update.} No credit update is applied until the
  ...
  at $\pi_0$. \hfill ace\_start\_frac, \_dt\_core.py:210--212
...
\item \textbf{Credit accumulation.} Every individual contributes its signed
  ...
  count. \hfill\_ace\_update\_probs, \_dt\_core.py:1203
...
\item \textbf{Mixing and projection.} The target is projected, mixed into $\pi$
  ...
  renormalization. \hfill\_ace\_project\_probs, \_dt\_core.py:694
\item \textbf{ARGP freeze.} After a warm-up fraction of the budget, an unfrozen
  ...
  single arm. \hfill\_argp\_update\_memory\_probs, \_dt\_core.py:1271
...
and a post-search cooldown.  The coordinate branch carries no confidence gate; the
...
suite (603 tests) and every release gate --- build hygiene,
...
The direct contribution of the interaction-structure memory --- the isolation
reviewers requested --- is a \emph{separate} experimental design from the
...
ranks over the four cells and Vargha--Delaney $A_{12}$ effect sizes on the raw
runs; the four cells differ overall (Iman--Davenport $F(3,84) = 5.59$,
...
$A_{12} = 0.59$, $0.53$, $0.64$); the contrast removes the whole phase, so which
of its parts carries the effect---and in particular whether the learned
eigenbasis outperforms coordinate axes---remains unidentified, while the adaptive confidence gate helps directionally but not
...
significant effect at these tiers (and the no\_sgsm row, in which the
...
specifically).  The interaction-structure memory's isolated
...
addendum (statistical\_analysis\_plan\_addendum\_cec2020\_lsgo.md,
signing commit 5c9bfae82) and binding on every sentence of this section.
...
\section{@@SECNUM!sec:supp:revision!S9.@@ Round-One Revision Experiments}\label{sec:supp:revision}

\noindent The four experiments in this section were requested by the round-one
reviewers to isolate mechanisms that the submitted manuscript had introduced
together.  Their designs, estimands, statistical conventions and the wording to
be used for each unfavourable outcome were registered in a signed addendum
before any of them produced a result; two subsequent deviations discovered
during post-hoc verification are recorded there as dated amendments and are
disclosed at the point of use below.  That addendum is published with the code
and data, as revision\_experiments\_preregistration.md under the review folder
of the repository named in the Data Availability Statement, so a reader can
...
two-contrast family; $A_{12}$ on the raw paired runs, computed with the first-named
arm as reference, so $A_{12} < 0.5$ favours the second.  W/T/L counts functions,
ties at a tolerance of $10^{-8}$.  $^{\dagger}$The coordinate-versus-none row is a
...
function shows the eigenframe arm four orders of magnitude behind.  At $D = 100$
the same direction appears without reaching significance
($p_{\mathrm{Holm}} = 0.054$).  The conclusion the paper draws is therefore
restricted to what was tested: at $D = 50$ on this suite, with this estimator and
...
governor is untouched, and the elite archive --- contrary to a statement in the
...
The population rule is not the source of the reported standing.  At the
...
comparison by transplant: the parameter set resolved at $D = 10$, and separately
the set resolved at $D = 100$, are applied unchanged at every dimension and
...
where it was demonstrated, and it identifies the $20 \le D < 50$ tier as
mis-specified rather than the tiering principle as unsound.  This localizes a
...
\subsection{@@SECNUM!sec:supp:rev:sensitivity!S9.4.@@ Parameter Sensitivity (Exploratory)}\label{sec:supp:rev:sensitivity}
...
column substituted; W/T/L counts functions where the shipped configuration beats
the perturbed one; Ratio is the median across functions of perturbed over
...
tested here is knife-edge under the levels executed; that includes the one
constant the project's own configuration record had previously flagged as
sensitive on a different suite.
