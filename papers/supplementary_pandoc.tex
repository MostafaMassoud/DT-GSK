...
\title{Supplementary Material for: DT-GSK: Dimension-Tiered Adaptive Configuration Selection and Deterministic Refinement for Gaining-Sharing Knowledge-Based Optimization Algorithm}
...
(function, dimension) cell, and $\mathrm{MaxFES} = 10^{4} \cdot D$.
...
$10^{7}$ at $D = 5, 10, 15, 20$ respectively.
...
test with Holm's step-down
correction at family level and $\alpha = 0.05$;
win/tie/loss counts use the tie rule $|\Delta| < 10^{-8}$.

Panel ranking uses the Friedman test with the
...
(BCa) bootstrap descriptive rank-stability intervals to the
...
comparators (GSK, \agsk{},
\apgsk{}, \fdbagsk{},
\atmals{} and \egsk{}) exactly
...
independently verified second-environment cell exists, the runtime
...
protocol, none of them analyzed here (Section~S7.1).  The standings are
...
further $D\ge 100$ extension), as in the main-text dimension-gating table.
...
of Equation~(@@REF!eq:opspec-conf@@)).  For each accepted move $i$ with strictly
...
require the graph-readiness condition of Equation~(@@REF!eq:opspec-conf@@), so it can
...
suite (618 tests) and every release gate (
...
$\mathrm{MaxFES} = 10^{4}\cdot D$, and $51$ runs per
...
test across functions, Holm-corrected
within each dimension's family of six disabled-cell
...
means removing the component degrades rank.  Colors follow the frozen
...
signed-rank test across the 29 functions with
Holm correction within each dimension's six-comparison
...
CEC2017 classes --- unimodal (F1, F3), simple multimodal
...
composition functions, whose subcomponent and rotation structure make variable
...
paper.  Statistics on this suite use the raw best
...
panel; they are not analyzed in this paper, they enter no ranking or test
...
competition's own: 10 functions at
...
AGSK was the runner-up of the CEC2020 competition, and
...
the runner-up, \dtgsk{} places fourth; the family
...
be used for each unfavorable outcome were registered in a signed addendum
...
arm as reference, so $A_{12} < 0.5$ favors the second; between-function scale
...
the executed levels are consequently larger than the registered twenty percent
...
twenty percent and are one-sided (registration amendment
A1).
...
error ratios within roughly two percent of unity throughout.  The single
exception is favorable and instructive (raising the population floor by one
