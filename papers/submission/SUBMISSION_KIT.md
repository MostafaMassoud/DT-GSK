...
Regenerated 2026-08-27 from the pass-42 / v2.15 REVISION-1 sources (the
...
this revised version to tag v2.15 — matching the manuscript's own Data
...
2. Do NOT rebuild anything in the repo afterwards. The five uploaded files are
   hash-recorded in `submission_package_manifest.json` at **v2.15**, and that
   record is what makes the submitted bytes checkable. Freeze pass-42 and tag
   v2.15 are the frozen state of this resubmission, exactly as pass-38 / v2.13
   is the frozen state of the original.
3. If a SECOND revision is requested, it becomes pass-43 through change
   control: a new freeze pass and a new superseding tag, never an edit to
   v2.15 in place (D-0045).
4. Two of the uploaded artifacts — the response letter and the change register —
   ...
   `git diff v2.13 v2.15` over the manuscript sources.
