...
Regenerated 2026-08-28 from the pass-49 / v2.22 REVISION-1 sources (the
...
this revised version to tag v2.22 — matching the manuscript's own Data
...
2. Do NOT rebuild anything in the repo afterwards. The five uploaded files are
   hash-recorded in `submission_package_manifest.json` at **v2.22**, and that
   record is what makes the submitted bytes checkable. Freeze pass-49 and tag
   v2.22 are the frozen state of this resubmission, exactly as pass-38 / v2.13
   is the frozen state of the original.
3. If a SECOND revision is requested, it becomes pass-50 through change
   control: a new freeze pass and a new superseding tag, never an edit to
   v2.22 in place (D-0045).
4. Two of the uploaded artifacts — the response letter and the change register —
   ...
   `git diff v2.13 v2.22` over the manuscript sources.
