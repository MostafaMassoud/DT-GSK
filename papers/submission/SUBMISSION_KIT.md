...
Regenerated 2026-08-29 from the pass-57 / v2.30 REVISION-1 sources (the
...
| **Response to reviewers** | `papers/submission/response_to_reviewers.pdf` AND `papers/submission/response_to_reviewers.docx` — both staged, both pinned untracked (they quote both reports, D-0049), both rendered from `papers/review_2026_08_24/response_to_reviewers.md`. **Restyled 2026-08-29 to the professor-supplied sample** (two reviewer sections, short answers pointing at pages/tables, the added Tables A43/A44/A45 and the revised Figure 4 reproduced inline, no status scaffolding); the fully-detailed technical version is preserved beside it as `response_to_reviewers_full_technical.md` (internal record, not for upload). Rebuild PDF: `SOURCE_DATE_EPOCH=1788220800 pandoc papers/review_2026_08_24/response_to_reviewers.md -o papers/submission/response_to_reviewers.pdf --pdf-engine=xelatex -V mainfont=Cambria -V "mainfontoptions=Ligatures=NoCommon" -V monofont=Consolas -V fontsize=11pt -V geometry:margin=2.4cm -V papersize=a4 -V colorlinks=true` (run from the repo root so the inline figure path resolves; `Ligatures=NoCommon` is load-bearing for copy-paste). Rebuild DOCX: `pandoc papers/review_2026_08_24/response_to_reviewers.md -o papers/submission/response_to_reviewers.docx`. Upload the PDF (or the DOCX if the form prefers Word); read page counts off the renders. |
...
this revised version to tag v2.30 — matching the manuscript's own Data
...
2. Do NOT rebuild anything in the repo afterwards. The five shipped artifacts
   ...
   `submission_package_manifest.json` at **v2.30** — that record is what makes
   ...
   are uploaded but hash-recorded nowhere. Freeze pass-57 and tag
   v2.30 are the frozen state of this resubmission, exactly as pass-38 / v2.13
...
4. Two of the uploaded artifacts — the response letter and the change register —
   ...
   `git diff v2.13 v2.30` over the manuscript sources.
