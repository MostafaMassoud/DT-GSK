...
| Transparent parameter-tuning effort | Baselines run reference constants (`optimizer_options: {}`; pop_size 100 where applicable); dt-gsk runs the frozen `pub` profile (`run_config.profile: "pub"`), a byte-locked faithful port of the paper-facing configuration (`_dt_profiles.py`, locked by `tests/unit/test_dt_profiles.py`); no opt-in candidate profile (`ism_profile` A/B/C) appears in any panel run_config | PASS |
