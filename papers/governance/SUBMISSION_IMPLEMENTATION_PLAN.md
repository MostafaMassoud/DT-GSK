...
- [x] **Word+Visio round-trip on `DT-GSK_visio.docx`** - **PASSED, author-confirmed 2026-08-01**: the flowchart opens in Visio for editing. First pass ever recorded for this gate; the cause of the two earlier failures was a wrong OPC relationship type, fixed in D-0041. The withdrawal note below is superseded and kept for the record. ~~*(withdrawn 2026-08-01)* retest gate. **Never retested; D4 is withdrawn (D-0040).** The first author test failed (OLE flowcharts rendered as static images in Word). On 2026-08-01 the artifact was rebuilt from the committed drawings and put through the new `papers/scripts/validate_visio_ole.py`, which passes every machine-checkable property - OPC parts, relationship targets, content types, the Visio namespace, page resolution, shape counts, OLE r:id resolution, byte-identity of each embedded drawing to its tracked source, and preview-image resolution - and is negative-tested against a missing part, a dangling target and malformed XML. So the packages are not malformed; if double-click still yields a static image the cause is OLE activation on the author's machine (Word OLE/ActiveX policy, Visio not installed, or the `Visio.Drawing.15` ProgID unregistered), not the artifact. Not pursued because the OLE DOCX is excluded from the submission package (C-001 5.4) and MDPI asks for figures as a >=600 dpi ZIP, which is built and shipped.
- [ ] fold OLE into default build + refreeze - **DELIBERATELY NOT DONE (2026-08-01, D-0042).** Its precondition (the round-trip) is now met, so this is a live option rather than a blocked one, but it is not taken: the OLE variant is excluded from the submission package (C-001 5.4), MDPI wants figures as a separate >=600 dpi ZIP (built and verified), and embedding OLE objects in the *submitted* DOCX would enlarge it, and risk an editor without Visio seeing an object they cannot open. Changing the shipped artifact now would also force another freeze pass for no submission benefit. Revisit only if a venue asks for editable diagrams.
- [WITHDRAWN 2026-08-01] ~~(opt) extension diagrams~~ - optional extension of a withdrawn phase.
