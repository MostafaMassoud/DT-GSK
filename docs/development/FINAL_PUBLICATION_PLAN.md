...
**R2 — Campaign command: `python run.py --config configs/family_cec2020.yml --workers 14`.** The build lens's `scripts/run_all_cec2020.py` was **overruled**: as verified when this plan was written, that wrapper hard-pointed at `configs/agsk_cec2020.yml` — AGSK-only, `seed_policy: reference` (not unified), workers 2, stale data root. One accidental invocation would have injected reference-schedule seeds into `results/_run_all/agsk/cec2020/per_run.csv` and silently broken the paired design. **This has since been discharged:** task 0.7's disposition landed under CR-0019 on 2026-07-28 — the wrapper was repointed and now reads `python run.py --config configs/family_cec2020.yml --workers 14` at `scripts/run_all_cec2020.py:9`, with its docstring recording the former target and the reason. The ruling's campaign command is unchanged; only the hazard it guarded against is gone. `--workers 14` is still mandatory (`src/gsk_family/runners/parallel.py:54-64` defaults to 2).
