...
|---|---|---|
| **§10.5 integrity sentinel** | ✅ **`runners/fp_regime.py:265-297`** — hard-verified **at every worker spawn**; `run_experiment.py:1397-1398, 1586-1587, 1634-1635, 1658-1659` passes it via `initargs` at every pool build; recorded in `environment.json`. Motivating witness in its docstring: CEC2017 F20 D10 seed 54241459 → `3.1217325598e-01` in fallback vs `0.0` fully-JIT | **This is the manual's §10.5 design already realized.** Its existence is exactly why Appendix A's *"make every fast path opt-in so the sentinel is invariant by construction"* is not optional advice here — it is enforced by a process that **fails the pool** |
