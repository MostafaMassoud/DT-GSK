...
| **Freeze** | **pass-56 / tag v2.29** (anchor `33ee170`) · `check_manifest` 15/15 + `sources 2/2` (now gating commit-field resolvability) · minted and tagged locally; push pending — verify `git ls-remote --tags origin`, the DAS names v2.29. Repo is **PRIVATE until upload day** — flip public before SuSy upload. Next free ids **CR-0037 / D-0062**. |
...
the current freeze is **pass-56 / v2.29** at 15/15 + sources 2/2 —
