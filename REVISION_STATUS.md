...
| **Freeze** | **pass-62 / tag v2.35** (anchor `934192a`) · `check_manifest` 15/15 + `sources 2/2` (gating commit-field resolvability) · minted and tagged 2026-08-29; push, then verify all 35 tags on `origin` (the DAS names v2.13 and v2.35). Repo is **PRIVATE until upload day** — flip public before SuSy upload. Next free ids **CR-0043 / D-0068**. |
...
the current freeze is **pass-62 / v2.35** at 15/15 + sources 2/2 — push, then verify both
...
2. **SuSy portal (author only)** --- the one thing left. Re-enter the new title and the revised
   ...
   marked copies and the register from `git diff v2.13 v2.35`. Paste text and the full
...
50, 53, 54, 56, 57, 58, 59, 60, 61 and 62, and the change register must come from `git diff v2.13 v2.35`. Read its passage count
