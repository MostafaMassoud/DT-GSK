...
| **Freeze** | **pass-59 / tag v2.32** (anchor `ed713a7`) · `check_manifest` 15/15 + `sources 2/2` (gating commit-field resolvability) · **PUSHED 2026-08-29** — all 32 tags on `origin`; v2.13 and v2.32, the two the DAS names, verified resolving there. Repo is **PRIVATE until upload day** — flip public before SuSy upload. Next free ids **CR-0040 / D-0065**. |
...
the current freeze is **pass-59 / v2.32** at 15/15 + sources 2/2 — push, then verify both
...
2. **SuSy portal (author only)** --- the one thing left. Re-enter the new title and the revised
   ...
   marked copies and the register from `git diff v2.13 v2.32`. Paste text and the full
...
6. **Make the repository PUBLIC again before uploading (author-only, 2026-09-01).** The author set it
   ...
   the revised manuscript names the repo URL and tags v2.13/v2.32, and the Supplementary's
...
agent-side is complete at pass-59 / `v2.32` (pass-51's acceptance-readiness fixes plus the
...
50, 53, 54, 56, 57, 58 and 59, and the change register must come from `git diff v2.13 v2.32`. Read its passage count
