...
| **Freeze** | **pass-67 / tag v2.40** (anchor `76ece26`); at proof stage the title became "...for Gaining-Sharing Knowledge-Based Optimization Algorithm" and both affiliations were corrected (postcode 12578; Mathematics Program). Previously **pass-66 / v2.39** (anchor `7729822`); the supplement re-sent to the journal in all three formats, aligned with the accepted article (affiliations corrected incl. postcode 12578, `post-hoc` -> `post hoc`). Previously **pass-65 / v2.38** (anchor `82f89b0`); the DAS deliberately stays at v2.37, the tag the paper in production prints (D-0070). Previously **pass-64 / v2.37** (anchor `725aa82`) · `check_manifest` 15/15 + `sources 2/2` (gating commit-field resolvability) · **PUSHED 2026-09-01** — all 37 tags on `origin`; v2.13, v2.36 and v2.37 verified resolving there (v2.37 also anonymously). The DAS names v2.13 and v2.37; v2.36 remains the frozen record of the 2026-08-30 SuSy resubmission. Repo is **PUBLIC** (verified anonymously 2026-09-01: repo, tags and pre-registration resolve; withheld files 404). Next free ids **CR-0045 / D-0070**. |
...
the current freeze is **pass-63 / v2.36** at 15/15 + sources 2/2 — push, then verify both
...
2. **SuSy portal (author only)** --- the one thing left. Re-enter the new title and the revised
   ...
   marked copies and the register from `git diff v2.13 v2.36`. Paste text and the full
...
agent-side is complete at pass-63 / `v2.36` (pass-51's acceptance-readiness fixes plus the
...
50, 53, 54, 56, 57, 58, 59, 60, 61, 62 and 63, and the change register must come from `git diff v2.13 v2.36`. Read its passage count
