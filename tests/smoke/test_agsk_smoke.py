...
def _agsk_options(seed: int, *, np_init: int = 20) -> dict[str, object]:
...
def test_agsk_sphere_smoke_and_result_contract() -> None:
    problem = make_problem("sphere", 1, dim=8, max_nfes_override=400)

    result = optimize(problem, _agsk_options(42))
...
def test_agsk_same_seed_replays_trajectory() -> None:
    problem = make_problem("sphere", 1, dim=6, max_nfes_override=280)

    first = optimize(problem, _agsk_options(77))
    second = optimize(problem, _agsk_options(77))
    different = optimize(problem, _agsk_options(78))
...
def test_agsk_fair_start_matches_internal_initialization_when_state_restored() -> None:
    ...
    direct = optimize(problem, _agsk_options(55))
...
def test_agsk_budget_crossing_scans_only_budgeted_child_prefix() -> None:
    ...
    result = optimize(problem, _agsk_options(12, np_init=12))
...
def test_agsk_target_stop_zeroes_error_after_generation_check() -> None:
    calls = 0
    ...
    result = optimize(problem, _agsk_options(19, np_init=12))
